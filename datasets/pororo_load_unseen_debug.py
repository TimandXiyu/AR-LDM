...
class StoryDataset(Dataset):
    ...
    def __init__(self, subset, args):
        ...
        self.target_char = args.get(args.dataset).target_chars

        self.data_dir = args.get(args.dataset).data_dir
        self.descriptions = np.load(os.path.join(self.data_dir, 'descriptions.npy'), allow_pickle=True, encoding='latin1').item()
        ...
        self.seen_train_len, self.seen_val_len, self.seen_test_len = len(self.h5_file['train']['text']), len(
            self.h5_file['val']['text']), len(self.h5_file['test']['text'])
        ...
        # Get unseen train and test samples from new followings JSON
        ...
        for i, img_ids in enumerate(self.story_list):
            if any(id.split(".")[0] in unseen_train_ids for id in img_ids):
                self.unseen_train.append(i)
            elif any(id.split(".")[0] in list(matching_img.keys()) for id in img_ids):
                self.unseen_test.append(i)

        self.unseen_train = list(set(self.unseen_train))
        self.unseen_test = list(set(self.unseen_test))
